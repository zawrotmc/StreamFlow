...
export default function StreamPage() {
  ...
  return (
    <div className="relative w-full h-screen bg-black overflow-hidden" data-testid="stream-page">
      ...
      {/* Stream Quality Indicator - Bottom Right */}
      <div className="absolute bottom-4 right-4 md:bottom-6 md:right-6 z-50">
        <div className="bg-black bg-opacity-60 backdrop-blur-sm rounded-lg px-3 py-2" data-testid="quality-indicator">
          <div className="flex items-center space-x-2">
            <div className={`w-2 h-2 rounded-full ${streamStatus.isLive ? 'bg-green-500' : 'bg-red-500'}`} />
            <span className="text-xs font-medium text-gray-300">
              {streamStatus.isLive ? '1080p' : 'Offline'}
            </span>
          </div>
        </div>
      </div>
